...
def parse_action(text: str) -> Optional[int]:
    ...
    for key in sorted(_STR_TO_ACTION.keys(), key=len, reverse=True):
        if key in text:
            return _STR_TO_ACTION[key]
    return None
...
def eval_slm_only(slm_cfg: Dict, size: int, n_episodes: int, seed_offset: int = 0):
    env = DoorKeyEnv(size=size)
...
def eval_ask(model: PPO, slm, size: int, threshold: float, n_episodes: int,
...
def objective(trial, model, slm, size, n_eval_episodes, n_mc_samples):
    threshold = trial.suggest_float("threshold", 0.01, 2.0)
    mean_reward, _ = eval_ask(model, slm, size, threshold, n_eval_episodes,
...
def main() -> None:
    ...
    if args.mode == "ppo":
        ...
        cfg_slm = {"env": f"DoorKey-{args.size}x{args.size}", "slm_model": model_name,
        ...
        if args.threshold is not None:
            ...
            _opt_slm = load_slm(cfg)
            study.optimize(
                lambda t: objective(t, _opt_model, _opt_slm, args.size,
