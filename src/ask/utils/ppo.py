...
class DropoutMlpExtractor(nn.Module):
    def __init__(self, feature_dim, net_arch, dropout_rate=0.2):
        ...
        for dim in net_arch["pi"]:
            policy_layers.extend([nn.Linear(last_dim, dim), nn.Dropout(p=dropout_rate), nn.Tanh()])
            last_dim = dim
        ...
        for dim in net_arch["vf"]:
            value_layers.extend([nn.Linear(last_dim, dim), nn.Dropout(p=dropout_rate), nn.Tanh()])
            last_dim = dim
